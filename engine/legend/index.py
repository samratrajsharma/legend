...
class ChromaRetriever:
    ...
    per-repo (legend_<repo-namespace>_<sig>) so a re-index after an edit can evict this
    ...
    def __init__(self, data_dir, sig="default", namespace="default"):
        ...
        # Disable Chroma's PostHog telemetry: it's off by principle (local-first) and its
        # background thread prints scary "Failed to send telemetry event" lines on some
        # chromadb versions, which is exactly the wrong first impression for `uvx legend`.
        try:
            _settings = chromadb.Settings(anonymized_telemetry=False)
        except Exception:
            _settings = None
        self.client = (
            chromadb.PersistentClient(path=os.path.join(data_dir, "chroma"), settings=_settings)
            if _settings is not None
            else chromadb.PersistentClient(path=os.path.join(data_dir, "chroma"))
        )
        self.ns = _collection_ns(namespace)
        self.prefix = f"legend_{self.ns}_"
